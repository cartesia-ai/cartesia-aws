# Sonic 3.5 Models on Sagemaker Jumpstart

### Getting Started

For a step by step guide to create your first Sonic 3.5 Inference endpoint on Sagemaker, please refer to [sample notebook](https://github.com/cartesia-ai/cartesia-aws/blob/main/sonic-3-5-jumpstart/Sonic-3-5-Jumpstart.ipynb)

### Inference Setup

Sonic 3.5 supports only real time inference on Sagemaker. Please select `ml.g6e.xlarge` or `ml.g7e.2xlarge` as your inference endpoint instance type. Each instance is capable of serving 10 concurrent requests. In order to get the best performance, Sagemaker suggests that you reuse the client-to-SageMaker connection, as it can save the time to re-establish the connection. In boto3, you can configure max_pool_connections . Multiple requests will reuse the connections, which avoids the cost of establishing new TCP/TLS connections for each request.
...
| **transcript** | The text that will be converted into speech. You can include additional controls (e.g., emotion, speed, volume) as supported by Sonic 3.5 models: https://docs.cartesia.ai/build-with-cartesia/sonic-3/volume-speed-emotion | `string` | ✅ Yes |
...
```json
{
    ...
    "transcript": "Welcome to Cartesia. Our Sonic 3.5 model delivers ultra-low latency TTS via the API.",
    "language": "en",
